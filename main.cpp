...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "TransmittanceModels", NULL, NULL);
    if (window == NULL)
    {
    ...
    Model suitModel(FileSystem::getPath("resources/objects/nanosuit/nanosuit.obj"));
    Model sphModel(FileSystem::getPath("resources/objects/planet/planet.obj"));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(-4.75f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
        model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));	// it's a bit too big for our scene, so scale it down
        shader.setMat4("model", model);
        problem = 4.0;
        shader.setFloat("problem", problem);
        suitModel.Draw(shader);

        
        model = glm::translate(model, glm::vec3(10.75f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
        ...
        model = glm::translate(model, glm::vec3(6.0f, 1.0f, 0.0f)); // translate it down so it's at the center of the scene
        model = glm::scale(model, glm::vec3(0.8f, 0.8f, 0.8f));	// it's a bit too big for our scene, so scale it down
        shader.setMat4("model", model);
        problem = 1.0;
        shader.setFloat("problem", problem);
        sphModel.Draw(shader);
